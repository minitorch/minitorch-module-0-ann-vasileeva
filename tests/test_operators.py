...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert (sigmoid(a) >= 0) & (sigmoid(a) <= 1)
    assert abs(1 - sigmoid(a) - sigmoid(-a)) < 1e-10
...
def test_distribute(x: float, y: float, z: float) -> None:
    ...
    """
    assert abs(mul(z, add(x,y)) - add(mul(z, x),mul(z, y))) < 1e-10
...
def test_other(x: float, y: float) -> None:
    ...
    if x < 10 and y < 10:
        assert abs(exp(add(x,y)) - mul(exp(x), exp(y))) < 1e-3
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert abs(sum(ls1) + sum(ls2) - sum(addLists(ls1, ls2))) < 1e-10
